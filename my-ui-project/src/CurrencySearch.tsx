...
function CurrencySearch() {
  ...
  const handleSearch = () => {
    ...
    apiClient.getCountryByCurrency(currency, fieldsToRequest) // Pass the selected fields
      .then(response => {
        setCountries(response.data);
      })
      .catch(err => {
        setError('Could not find a country with that currency.');
  ...
  return (
    <div>
      <h2>Search by Capital Currency</h2>
      <input
        type="text"
        value={currency}
