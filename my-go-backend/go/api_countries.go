...
func (api *CountriesAPI) GetCountryByCapital(c *gin.Context) {
	...
	if cached, ok := countryCache.Get(cacheKey); ok { c.JSON(http.StatusOK, cached); return }
	client := newClient()
	helper := client.CountriesAPI.GetCountryByCapital(context.Background(), capital)
	if fields != "" { helper = helper.Fields(strings.Split(fields, ",")) }
	countries, _, err := helper.Execute()
	if err != nil { handleExternalError(c, err, fmt.Sprintf("capital '%s'", capital)); return }
	if len(countries) == 0 { c.JSON(http.StatusNotFound, gin.H{"error": fmt.Sprintf("No countries found for capital '%s'", capital)}); return }
...
func (api *CountriesAPI) GetCountryByCode(c *gin.Context) {
	...
	if fields != "" { helper = helper.Fields(strings.Split(fields, ",")) }
	countries, _, err := helper.Execute()
	if err != nil { handleExternalError(c, err, fmt.Sprintf("code '%s'", code)); return }
	if len(countries) == 0 { c.JSON(http.StatusNotFound, gin.H{"error": fmt.Sprintf("No countries found for code '%s'", code)}); return }
	countryCache.Set(cacheKey, countries)
	c.JSON(http.StatusOK, countries)
}
...
func (api *CountriesAPI) GetCountryByCurrency(c *gin.Context) {
	...
	if fields != "" { helper = helper.Fields(strings.Split(fields, ",")) }
	countries, _, err := helper.Execute()
	if err != nil { handleExternalError(c, err, fmt.Sprintf("currency '%s'", currency)); return }
	if len(countries) == 0 { c.JSON(http.StatusNotFound, gin.H{"error": fmt.Sprintf("No countries found for currency '%s'", currency)}); return }
	countryCache.Set(cacheKey, countries)
	c.JSON(http.StatusOK, countries)
}
...
func (api *CountriesAPI) GetCountryByLanguage(c *gin.Context) {
	...
	if fields != "" { helper = helper.Fields(strings.Split(fields, ",")) }
	countries, _, err := helper.Execute()
	if err != nil { handleExternalError(c, err, fmt.Sprintf("language '%s'", language)); return }
	if len(countries) == 0 { c.JSON(http.StatusNotFound, gin.H{"error": fmt.Sprintf("No countries found for language '%s'", language)}); return }
	countryCache.Set(cacheKey, countries)
	c.JSON(http.StatusOK, countries)
}
...
func (api *CountriesAPI) GetCountryByName(c *gin.Context) {
	...
	if fields != "" { helper = helper.Fields(strings.Split(fields, ",")) }
	countries, _, err := helper.Execute()
	if err != nil { handleExternalError(c, err, fmt.Sprintf("name '%s'", name)); return }
	if len(countries) == 0 { c.JSON(http.StatusNotFound, gin.H{"error": fmt.Sprintf("No countries found for name '%s'", name)}); return }
	countryCache.Set(cacheKey, countries)
	c.JSON(http.StatusOK, countries)
}
...
func (api *CountriesAPI) GetCountryByRegion(c *gin.Context) {
	...
	if fields != "" { helper = helper.Fields(strings.Split(fields, ",")) }
	countries, _, err := helper.Execute()
	if err != nil { handleExternalError(c, err, fmt.Sprintf("region '%s'", region)); return }
	if len(countries) == 0 { c.JSON(http.StatusNotFound, gin.H{"error": fmt.Sprintf("No countries found for region '%s'", region)}); return }
	countryCache.Set(cacheKey, countries)
	c.JSON(http.StatusOK, countries)
}
...
func (api *CountriesAPI) GetCountryBySubregion(c *gin.Context) {
	...
	if fields != "" { helper = helper.Fields(strings.Split(fields, ",")) }
	countries, _, err := helper.Execute()
	if err != nil { handleExternalError(c, err, fmt.Sprintf("subregion '%s'", subregion)); return }
	if len(countries) == 0 { c.JSON(http.StatusNotFound, gin.H{"error": fmt.Sprintf("No countries found for subregion '%s'", subregion)}); return }
	countryCache.Set(cacheKey, countries)
	c.JSON(http.StatusOK, countries)
}
...
func (api *CountriesAPI) GetCountryByTranslation(c *gin.Context) {
	...
	if fields != "" { helper = helper.Fields(strings.Split(fields, ",")) }
	countries, _, err := helper.Execute()
	if err != nil { handleExternalError(c, err, fmt.Sprintf("translation '%s'", translation)); return }
	if len(countries) == 0 { c.JSON(http.StatusNotFound, gin.H{"error": fmt.Sprintf("No countries found for translation '%s'", translation)}); return }
	countryCache.Set(cacheKey, countries)
	c.JSON(http.StatusOK, countries)
}
...
func (api *CountriesAPI) GetIndependentCountries(c *gin.Context) {
	...
	if fields != "" { helper = helper.Fields(strings.Split(fields, ",")) }
	countries, _, err := helper.Execute()
	if err != nil { handleExternalError(c, err, "independent countries"); return }
	if len(countries) == 0 { c.JSON(http.StatusNotFound, gin.H{"error": "No independent countries found"}); return }
	countryCache.Set(cacheKey, countries)
	c.JSON(http.StatusOK, countries)
}

func HealthCheck(c *gin.Context) {
	resp, err := http.Get("https://restcountries.com/v3.1/alpha/us")
	if err != nil || resp.StatusCode != http.StatusOK {
		c.JSON(http.StatusServiceUnavailable, gin.H{"status": "unhealthy", "reason": "cannot reach restcountries API", "timestamp": time.Now().UTC().Format(time.RFC3339)})
		return
	}
